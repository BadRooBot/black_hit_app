...
class _TapJoyState extends State<TapJoy> {
  ...
  void initState() {
    ...
    TapJoyPlugin.shared.connect(
        androidApiKey:"uHRlVCNCRJ6aohUKzZveuAECbG1agxg30fJHXsAOZ8fnylVaDUsp4IZDsuus",// "uHRlVCNCRJ6aohUKzZveuAECbG1agxg30fJHXsAOZ8fnylVaDUsp4IZDsuus",
        iOSApiKey: "k_X8qU2XQwqE6WZ9g_IEyAEBEVKcOcwAUfyTbP3FeYi2tKW_KrbNqvKs4u-_",
        debug: true);
    // set userID
...
// currency handler
            ...
            children: [
              ...
              Container(padding: EdgeInsets.symmetric(horizontal: 15,vertical: 5),
                  child: GestureDetector(onTap: (){
                    ShowOfferWall(context);
                  },child: ClipRRect(borderRadius:  BorderRadius.circular(6),child: Image.asset("assets/offer.png",fit: BoxFit.fill,width: size.width-15,height: 150,),)))
             , Container(padding: EdgeInsets.symmetric(horizontal: 15,vertical: 15),
                  child: GestureDetector(onTap: (){
                    ShowVideo(context);
                  },child: ClipRRect(borderRadius:  BorderRadius.circular(6),child: Image.asset("assets/watch_video.png",fit: BoxFit.fill,width: size.width-15,height: 150,),)))


            ],
          ),
        ) :Center(child: Text("Sorry you have to download one land app to use this feature")),
  ...
  ShowOfferWall(context) async{
      if(isRReady){
            ...
            await  myPlacement.requestContent.call().whenComplete(() => isRReady=true);
  ...
  ShowVideo(context) async{
    if(isRReady){
      ...
      showLoading(context, "", true);
      await  myPlacement2.requestContent.call().whenComplete(() => isRReady=true);
