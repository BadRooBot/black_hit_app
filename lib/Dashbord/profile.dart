...
import 'package:AliStore/login/login.dart';
...
class profile extends StatefulWidget {
  final uid, VIP;
  const profile({Key? key, required this.uid, required this.VIP})
      : super(key: key);
...
class _profileState extends State<profile> {
  _selectImage(BuildContext parentContext) async {
    ...
    String changeprofilepicture =
        getTranslated(context, "changeprofilepicture");
  ...
  getData() async {
    ...
    try {
      //userData.clear();
      isMe = false; //widget.uid == myInfoList[0]["id"] ? true : false;
...
*/
      ...
      dynamic _val = await API().getOneUser(widget.uid);
      ...
      // get post lENGTH
      //var postSnap = await API().getMyPosts(widget.uid);
      ...
      print(userSnap);
      Name = _val["username"];
      ImageUri = _val["image_url"];
      Status = _val["gender"];
      /*  followers = userSnap['followers'].length;
        following = userSnap['following'].length;*/

      // followers = await realtimeDb().GetFollowers(
      //         Id: widget.uid ?? FirebaseAuth.instance.currentUser!.uid,
      //         NeedfollowersCount: true) ??
      //     0;
      // following = await realtimeDb().GetFollowing(
      //         Id: widget.uid ?? FirebaseAuth.instance.currentUser!.uid,
      //         NeedfollowingCount: true) ??
      //     0;
      // isFollowing = await realtimeDb().GetFollowers(
      //         Id: widget.uid ?? FirebaseAuth.instance.currentUser!.uid,
      //         NeedfollowersCount: false) ??
      //     false;
    } catch (e) {
      print('=========================Eror $e');
      // showSnackBar(
  ...
  Widget build(BuildContext context) {
    String ProfileBackImage =
        'https://img-prod-cms-rt-microsoft-com.akamaized.net/cms/api/am/imageFileData/RE4wyTI?ver=c51c';
            ...
            : ListView(
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      buildStatColumn(followers, "$_followers"),
                      buildStatColumn(following, "$_following"),
                    ],
                  ),
                  SizedBox(height: 70),
                  Row(
                  ...
                  Row(
                    ...
                    await FireStoreMethods()
                      ...
                      userData['uID'],
                                ...
                                // await realtimeDb().AddFollows(
                                //     Id: widget.uid, followers: true);
                                // await realtimeDb().AddFollowing(
                                // Id: widget.uid, followers: true);
                  ...
                  FutureBuilder(
                    future: API().getMyPosts(widget.uid),
                      ...
                      return Card(
                        child: GridView.builder(
                          padding: EdgeInsets.only(
                              left: 12, right: 12, bottom: 0, top: 5),
                          shrinkWrap: true,
...
*/
                            await API().updateUserUsername(
                                myInfoList[0]["id"], _usernameController.text);
                            setState(() {
                              Name = _usernameController.text;
                              myInfoList[0]["username"] =
                    ...
                    children: [
                      ...
                      ElevatedButton(
                          onPressed: () async {
                            ...
                            //       await editStatusDb.EditNameAndStatus(
                            //           NewName: 'bad RooBot',
                            //           NewStatus: _bioController.text,
                            //           NameOrStatus: false);
                            //       setState(() {
...
//     var ImUrl = await StorageMethods().uploadImageGroupToStorage(
//         "ImageProfile", Filess, "${FirebaseAuth.instance.currentUser!.uid}");
//     /* await  FirebaseFirestore.instance.collection("users").doc("${FirebaseAuth.instance.currentUser!.uid}").
