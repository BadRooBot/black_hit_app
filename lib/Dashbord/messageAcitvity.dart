...
class _messageState extends State<message> {
  ...
  void sendMessage(yourName, yourProfilePic) async {
    try {
      String res =API().          /*await FireStoreMethods().sendMessage(
        ...
        if (NotEx) {
          ...
          if (widget.isGroup) {
            /*var nAndI = await _firestore.collection('users')
                ...
                isEqualTo: AllId[num])
                .get();*/
            List nAndI = await realtimeDb().GetOneUserData(Id: AllId[num]);
            ...
            nAndI.forEach((element) async {
              ...
              setState(() {
                ...
                print(
                    '++++++++++++++++Enter length done   ${userInfoMessage.length}');
              });
            });
          }
        } else {
  ...
  GetAllIds() async {
    Update = false;
    var _getAllIds =
        await _firestore.collection('chatList').doc(widget.Uid).get();
                      ...
                      GetAllImage(index) {
                        if (userInfoMessage.isNotEmpty) {
                          for (int xx = 0; xx < userInfoMessage.length; xx++) {
                            if (userInfoMessage[xx]["uID"] ==
                                snapshot.data!.docs[index]["uid"]) {
                              YourImageeee =
                                  userInfoMessage[xx]["imageProFile"];
                              return YourImageeee;
                            }
                          }
                        } else
            ...
            child: Container(
              ...
              child: InkWell(
                  ...
                  child: const Icon(Icons
                      .send_outlined)), //Text('Send', style: TextStyle(color: Colors.lightBlue),)
            ),
          )
        ]),
      ),
  ...
  String getTimeAgo(int time) {
    ...
    var now =
        Timestamp.now().millisecondsSinceEpoch; //DateTime.now().millisecond;
    ...
    if (diff < MINUTE_MILLIS) {
      ...
      return '${(finls)}' + " minutes ago";
    } else if (diff < 60 * MINUTE_MILLIS) {
      return "an hour ago";
    } else if (diff < 24 * HOUR_MILLIS) {
      finls = (diff / HOUR_MILLIS);
      return '${(finls)}' + " hours ago";
    } else if (diff < 48 * HOUR_MILLIS) {
      return "yesterday";
    } else {
      finls = (diff / DAY_MILLIS);

      return '${(finls)}' + " days ago";
    }
  }
  ...
  DeleteView(
    ...
    var DeleteOneMessageForAll =
        getTranslated(context, "DeleteOneMessageForAll");
    var Cancel = getTranslated(context, "Cancel");
    var OK = getTranslated(context, "OK");
    var ConfirmOneMessageDataForMe =
        getTranslated(context, "ConfirmOneMessageDataForMe");
    var ConfirmOneMessageDataForMeAndYou =
        getTranslated(context, "ConfirmOneMessageDataForMeAndYou");
  ...
  yourMessage(snapshot, index, ImageProfile, YourName, bool VIPs, List listNM) {
    ...
    if (index != snapshot.data?.docs.length - 1) {
      ...
      lastIsMe = snapshot.data?.docs[index + 1]['senderId'] ==
      ...
      onLongPress: () {
        if (snapshot.data?.docs[index]['isGroup']) {
          if (snapshot.data?.docs[index]['senderId'] ==
              FirebaseAuth.instance.currentUser!.uid) {
            DeleteView(
                MessageID: snapshot.data?.docs[index]['messageKey'],
      ...
      child: Stack(
        ...
        children: [
              ...
              child: Container(
                decoration: BoxDecoration(
                  borderRadius: BorderRadius.only(
                      ...
                      bottomLeft: Radius.circular(Allme),
                      bottomRight: Radius.circular(Allme2)),
          ...
          Visibility(
            visible: myOrNot,
            child: Visibility(
                visible: snapshot.data?.docs[index]['isGroup'],
                child: Positioned(
                    bottom: 11,
                    left: 4,
            ...
            child: Visibility(
                ...
                child: Positioned(
                    ...
                    child: Visibility(
                        visible: iSendLastMessage,
                        child:
                            Opacity(opacity: .60, child: Text('$YourName'))))),
          ),
          Visibility(
              visible: iSendLastMessage,
              child: Positioned(
                  ...
                  child: Opacity(
                      opacity: .15,
                      child: Text(
                          '${timeago.format(snapshot.data!.docs[index]['datePublished'].toDate(), locale: LangCod.contains('ar') ? 'ar' : 'en')}'))))
        ],
      ),
    );
  }
              ...
              child: Container(
                decoration: BoxDecoration(
                  borderRadius: BorderRadius.only(
                      topLeft: Radius.circular(15),
                      topRight: Radius.circular(15),
                      bottomLeft: Radius.circular(Allme2),
                      bottomRight: Radius.circular(Allme)),
          ...
          Visibility(
              ...
              child: Positioned(
                  ...
                  child: Opacity(
                      opacity: .15,
                      child: Text(
                          '${timeago.format(snapshot.data!.docs[index]['datePublished'].toDate(), locale: LangCod)}'))))
        ],
      ),
    );
...
ShowIcon(index, ImageProfile, YourName, bool IsVip) {
  return IsVip
      ...
      : CircleAvatar(
          ...
          radius: 12,
          child: Text(
            "${YourName.toString().substring(0, 1).toUpperCase()}",
